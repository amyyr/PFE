...
var videoDecoder1 = new VideoDecoder({
  ...
  function processVideo(frame){
    ...
    Promise.all([
            ...
            if(framestart>= parseInt(filterarray[i].startvalue)&&framestart<= parseInt(filterarray[i].endvalue)){
              ...
              if(filterarray[i].type ==="brightness"){
                ...
                imgInstance.filters.push(new fabric.Image.filters.Brightness(
                  ...
                  {
                    brightness: intensity // Brightness value (0 to 2)
                }
              ...
              if(filterarray[i].type ==="contrast"){
                ...
                imgInstance.filters.push(new fabric.Image.filters.Contrast(
                  ...
                  {
                    contrast: intensity // Contrast value (-100 to 100)
                }
                ));
              ...
              if(filterarray[i].type ==="saturation"){

                imgInstance.filters.push(new fabric.Image.filters.Saturation(

                  {
                    saturation: intensity // Contrast value (-100 to 100)
                }
                ));
        ...
        if(videoinfo.width>500){

        imgInstance.scaleToWidth(videoinfo.width/1.5)
        imgInstance.scaleToHeight(videoinfo.height/1.5)
      }

      else{
        imgInstance.scaleToWidth(videoinfo.width)
        imgInstance.scaleToHeight(videoinfo.height)
...
drawing[i].set(drawing[i].animationarray[j].originX, drawing[i].animationarray[j].origin)
drawing[i].set(drawing[i].animationarray[j].originY, drawing[i].animationarray[j].origin)
...
document.getElementById("error").innerHTML = "Ending Point is greater than equal to Starting Point"
      ...
      for(i=0;i<drawing.length;i++){
        ...
        drawing[i].set({
          ...
          top:drawing[i].top /1.5,
          left:drawing[i].left /1.5,
          scaleX:drawing[i].scaleX/1.5,
          scaleY:drawing[i].scaleY/1.5,
          skewX:drawing[i].skewX/1.5,
          skewY: drawing[i].skewY/1.5,
...
if(videoinfo.width>500){
      canvas1212.width = videoinfo.width/1.5
      canvas1212.height = videoinfo.height/1.5
    }
